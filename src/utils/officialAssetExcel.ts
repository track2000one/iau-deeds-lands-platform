...
const setValue = (doc: XMLDocument, cell: Element, value: unknown) => {
...
const getCellText = (cell: Element, sharedStrings: string[]) => {
  ...
  if (type === 'inlineStr') {
    return Array.from(cell.getElementsByTagNameNS(XML_NS, 't')).map((item) => item.textContent || '').join('');
  }
  const v = directChild(cell, 'v')?.textContent || '';
  if (type === 's') return sharedStrings[Number(v)] || '';
  return v;
};

const cloneTemplateRow = (doc: XMLDocument, sourceRow: Element, rowNumber: number) => {
  ...
  Array.from(cloned.children).forEach((child) => {
    ...
    if (formula?.textContent) {
      const sourceNumber = Number(sourceRow.getAttribute('r') || rowNumber);
      formula.textContent = formula.textContent.replace(new RegExp(`(?<=[A-Z])${sourceNumber}(?!\\d)`, 'g'), String(rowNumber));
    }
  });
  return cloned;
};

const ensureRow = (doc: XMLDocument, sheetData: Element, templateRow: Element, rowNumber: number) => {
  const existing = rowElement(sheetData, rowNumber);
  if (existing) return existing;
  const row = cloneTemplateRow(doc, templateRow, rowNumber);
  const laterRow = Array.from(sheetData.children).find((child) => child.localName === 'row' && Number(child.getAttribute('r')) > rowNumber);
  if (laterRow) sheetData.insertBefore(row, laterRow);
  else sheetData.appendChild(row);
  return row;
};

const parseSharedStrings = async (zip: JSZip) => {
  ...
  return Array.from(doc.getElementsByTagNameNS(XML_NS, 'si')).map((si) =>
    Array.from(si.getElementsByTagNameNS(XML_NS, 't')).map((t) => t.textContent || '').join('')
  );
};

type WorkbookSheet = { name: string; relationshipId: string; path: string };

const workbookSheets = (workbookXml: string, relsXml: string): WorkbookSheet[] => {
  const parser = new DOMParser();
...
const findSheet = (sheets: WorkbookSheet[], config: SheetConfig) => {
  const keywords = config.sheetKeywords.map(normalize);
...
const findHeaderRow = (sheetData: Element, sharedStrings: string[], payloadKeys: Set<string>) => {
  const normalizedKeys = new Set(Array.from(payloadKeys).map(normalize));
  let best: { rowNumber: number; score: number; columns: Map<string, string> } | null = null;
  for (const row of Array.from(sheetData.children).filter((child) => child.localName === 'row').slice(0, 25)) {
    ...
    for (const cell of Array.from(row.children).filter((child) => child.localName === 'c')) {
      const header = getCellText(cell, sharedStrings).trim();
      if (!header) continue;
      const normalizedHeader = normalize(header);
      if (!normalizedKeys.has(normalizedHeader)) continue;
      const column = columnFromRef(cell.getAttribute('r') || '');
      if (!column) continue;
      const payloadKey = Array.from(payloadKeys).find((key) => normalize(key) === normalizedHeader) || header;
      columns.set(column, payloadKey);
      score += 1;
    }
    if (!best || score > best.score) best = { rowNumber, score, columns };
  }
  return best && best.score > 0 ? best : null;
};

const fallbackPayload = (asset: AssetRecord): Record<string, unknown> => ({
  'اسم الجهة': first(asset.entityName, asset.department),
  'رمز الجهة': asset.entityCode,
  'وصف الأصل': first(asset.assetDescription, asset.name),
  'رقم البطاقة': first(asset.cardNumber, asset.barcode, asset.itemNumber, asset.assetNumber),
  'القسم/الإدارة المسؤولة أو الشخص المسؤول': first(asset.responsibleDepartment, asset.department),
  'المنطقة': asset.region,
  'المدينة': asset.city,
  'رقم المبنى': first(asset.buildingNumber, asset.building),
  'الإحداثيات': asset.coordinates,
  'العمر المتبقي': asset.remainingLife,
  'العمر الإنتاجي': asset.usefulLife,
  'تكلفة الاقتناء': asset.acquisitionCost ?? asset.purchaseValue,
  'الوثائق الداعمة لتكلفة الاقتناء': asset.supportingCostDocument,
  'رقم أرشفة وثيقة إثبات الأصل': asset.archiveDocumentNumber,
  'المصنع': first(asset.manufacturer, asset.brand),
  'حالة الأصل': asset.technicalCondition,
  'وحدة القياس': asset.unitOfMeasure,
  'العدد': asset.quantity,
  'رقم الدور': asset.floor,
  'رقم الغرفة/ المكتب': asset.room,
});

const effectivePayload = (asset: AssetRecord) => ({ ...fallbackPayload(asset), ...payloadOf(asset) });

const writeGenericSheet = async (
  zip: JSZip,
  sheet: WorkbookSheet,
  assets: AssetRecord[],
  ...
  if (!file || !assets.length) return { exported: 0, warning: '' };
  const parser = new DOMParser();
  const doc = parser.parseFromString(await file.async('string'), 'application/xml');
  if (doc.getElementsByTagName('parsererror').length) throw new Error(`تعذر تحليل ورقة ${sheet.name}.`);
  const sheetData = doc.getElementsByTagNameNS(XML_NS, 'sheetData')[0];
  if (!sheetData) throw new Error(`ورقة ${sheet.name} لا تحتوي منطقة بيانات صالحة.`);

  const payloadKeys = new Set<string>();
  assets.forEach((asset) => Object.keys(effectivePayload(asset)).filter((key) => !key.startsWith('__')).forEach((key) => payloadKeys.add(key)));
  const header = findHeaderRow(sheetData, sharedStrings, payloadKeys);
  if (!header) return { exported: 0, warning: `تعذر تحديد عناوين الأعمدة في ورقة ${sheet.name}.` };

  const firstDataRow = header.rowNumber + 1;
  const existingRows = Array.from(sheetData.children).filter((child) => child.localName === 'row' && Number(child.getAttribute('r')) >= firstDataRow);
  const templateRow = existingRows[0] || rowElement(sheetData, header.rowNumber);
  if (!templateRow) throw new Error(`تعذر تحديد صف القالب في ورقة ${sheet.name}.`);

  const writableColumns = Array.from(header.columns.keys());
  ...
  assets.forEach((asset, index) => {
    ...
    const row = ensureRow(doc, sheetData, templateRow, rowNumber);
    const payload = effectivePayload(asset);
    header.columns.forEach((payloadKey, column) => {
      const cell = ensureCell(doc, row, column, rowNumber);
      const value = payload[payloadKey];
      setValue(doc, cell, value ?? '');
    });
  });
  ...
  if (dimension) {
    ...
    const endColumn = (ref.split(':').pop()?.match(/^[A-Z]+/) || [''])[0];
    if (endColumn) dimension.setAttribute('ref', `A1:${endColumn}${Math.max(firstDataRow + assets.length - 1, firstDataRow)}`);
  }

  zip.file(sheet.path, new XMLSerializer().serializeToString(doc));
  return { exported: assets.length, warning: '' };
};

const writeEquipmentSheet = async (
  zip: JSZip,
  sheet: WorkbookSheet,
  assets: AssetRecord[]
) => {
  ...
  const templateRow = existingRows[0] || rowElement(sheetData, 8);
  if (!templateRow) throw new Error('تعذر تحديد صف قالب الآلات والمعدات.');

  existingRows.forEach((row) => {
    const rowNumber = Number(row.getAttribute('r') || 0);
    [...MANUAL_GREEN_COLUMNS, ...FORMULA_GREEN_COLUMNS].forEach((column) => {
      const cell = cellElement(row, `${column}${rowNumber}`);
      if (cell) clearCell(cell);
    });
  });
  ...
  assets.forEach((asset, index) => {
    ...
    const row = ensureRow(doc, sheetData, templateRow, rowNumber);
    const values = equipmentValueMap(asset);
    MANUAL_GREEN_COLUMNS.forEach((column) => {
      const cell = ensureCell(doc, row, column, rowNumber);
      setValue(doc, cell, values[column] ?? '');
    });
    const formulas = equipmentFormulaMap(rowNumber);
    FORMULA_GREEN_COLUMNS.forEach((column) => {
      const cell = ensureCell(doc, row, column, rowNumber);
...
export type OfficialAssetExcelSheetResult = {
  kind: string;
  label: string;
  sheetName: string;
...
export type OfficialAssetExcelExportResult = {
  ...
  exportedCount: number;
  skippedCount: number;
  sheets: OfficialAssetExcelSheetResult[];
  warnings: string[];
};
...
export async function buildOfficialAssetExcel(
  ...
  const grouped = new Map<ExportKind, AssetRecord[]>();
  let skippedCount = 0;
  ...
  const sheetResults: OfficialAssetExcelSheetResult[] = [];
  const warnings: string[] = [];
  let exportedCount = 0;

  for (const config of SHEET_CONFIGS) {
    const records = grouped.get(config.kind) || [];
    if (!records.length) continue;
    const sheet = findSheet(sheets, config);
    if (!sheet) {
      skippedCount += records.length;
      warnings.push(`لم يتم العثور على ورقة ${config.label} داخل القالب الرسمي، لذلك تم تجاوز ${records.length} سجلًا.`);
      continue;
    }
    const result = config.kind === 'equipment'
      ? await writeEquipmentSheet(zip, sheet, records)
      : await writeGenericSheet(zip, sheet, records, sharedStrings);
    exportedCount += result.exported;
    skippedCount += records.length - result.exported;
    if (result.warning) warnings.push(result.warning);
    sheetResults.push({ kind: config.kind, label: config.label, sheetName: sheet.name, exportedCount: result.exported });
  }
