      ...
      <Card className="overflow-hidden rounded-[32px] border-sky-900/20 bg-gradient-to-l from-[#10243b] via-[#153654] to-[#0b4a67] text-white shadow-[0_26px_75px_rgba(15,23,42,.2)]">
        <CardContent className="p-6 sm:p-8">
          <div className="flex flex-col gap-5 lg:flex-row lg:items-center lg:justify-between">
            <div>
              <div className="mb-3 flex flex-wrap gap-2"><Badge className="border-white/15 bg-white/10 text-white"><History className="ml-1 h-3.5 w-3.5" />سجل تاريخي كامل</Badge><Badge className="border-emerald-300/25 bg-emerald-300/15 text-emerald-50"><ShieldCheck className="ml-1 h-3.5 w-3.5" />لا حذف للبيانات القديمة</Badge></div>
              <h1 className="text-3xl font-black sm:text-4xl">دورات تحديث بيانات الأصول</h1>
              <p className="mt-2 max-w-3xl text-sm leading-7 text-slate-200">كل تحديث يحفظ كإصدار مستقل. الدورة الجديدة تمر بالاستيراد والمقارنة والمراجعة قبل أن تصبح هي البيانات الحالية، وتبقى الإصدارات السابقة مؤرشفة وقابلة للعرض والتقرير.</p>
            </div>
            <div className="flex flex-wrap gap-2">
              {canAdd && <Button className="bg-cyan-400 text-slate-950 hover:bg-cyan-300" onClick={() => openCycle ? navigate(`/assets/import?cycleId=${openCycle.id}`) : setShowCreate(true)}><PlusCircle className="ml-2 h-4 w-4" />{openCycle ? 'استكمال الدورة المفتوحة' : 'إنشاء دورة جديدة'}</Button>}
              <Button variant="outline" className="border-white/20 bg-white/10 text-white hover:bg-white/15 hover:text-white" onClick={() => navigate('/assets')}><ArrowLeft className="ml-2 h-4 w-4" />لوحة الأصول</Button>
            </div>
          </div>
          {currentCycle && <div className="mt-6 rounded-2xl border border-white/15 bg-white/10 p-4"><div className="text-xs font-bold text-slate-300">الدورة الحالية المعتمدة</div><div className="mt-1 text-lg font-black">#{currentCycle.cycleNumber} — {currentCycle.name}</div><div className="mt-2 text-xs text-slate-300">{currentCycle.recordCount.toLocaleString('ar-SA')} سجل — اعتمدت في {formatDate(currentCycle.approvedAt)}</div></div>}
        </CardContent>
      </Card>

      {showCreate && canAdd && !openCycle && <Card className="rounded-[28px] border-sky-200 bg-white/90"><CardHeader><CardTitle>إنشاء دورة تحديث جديدة</CardTitle></CardHeader><CardContent className="grid gap-4 lg:grid-cols-2"><div><label className="mb-2 block text-xs font-bold">اسم الدورة</label><Input value={createName} onChange={(e) => setCreateName(e.target.value)} placeholder="مثال: تحديث بيانات الأصول — الربع الثالث 2026" /></div><div><label className="mb-2 block text-xs font-bold">وصف مختصر</label><Textarea value={createDescription} onChange={(e) => setCreateDescription(e.target.value)} className="min-h-10" placeholder="مصدر البيانات أو سبب التحديث" /></div><div className="flex gap-2 lg:col-span-2"><Button onClick={() => void handleCreate()} disabled={busyId === 'create'}>{busyId === 'create' ? <Loader2 className="ml-2 h-4 w-4 animate-spin" /> : <PlusCircle className="ml-2 h-4 w-4" />}إنشاء والانتقال للاستيراد</Button><Button variant="outline" onClick={() => setShowCreate(false)}>إلغاء</Button></div></CardContent></Card>}
