...
export const LeasedBuildingsInPage: React.FC = () => {
  ...
  const { leasedBuildingsIn, deleteLeasedBuildingIn } = useData();
  const { hasPermission } = usePermissions();
  const [searchQuery, setSearchQuery] = useState('');

  const filteredBuildings = leasedBuildingsIn.filter((building) =>
  ...
  return (
    <div className="container mx-auto p-6 space-y-6">
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-3xl font-bold">{t('buildings.leasedIn')}</h1>
          <p className="text-muted-foreground">{t('buildings.descriptionLeasedIn')}</p>
        </div>
        {hasPermission('leased_buildings_in', 'canAdd') && (
          <Button onClick={() => navigate('/buildings/leased-in/new')}>
                ...
                {filteredBuildings.length === 0 ? (
                  <TableRow>
                    <TableCell colSpan={8} className="text-center text-muted-foreground">
                      {t('buildings.noRentedBuildings')}
                    </TableCell>
                  </TableRow>
                ) : (
                  filteredBuildings.map((building) => (
                    <TableRow key={building.id}>
                      <TableCell className="font-medium">{building.owner.name}</TableCell>
