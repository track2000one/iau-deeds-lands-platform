...
export const LeasedBuildingsOutPage: React.FC = () => {
  ...
  const {
    ...
    updateLeasedBuildingOut,
    deleteLeasedBuildingOut,
  } = useData();
  ...
  const [selectedRecord, setSelectedRecord] = useState<LeasedBuildingOut | null>(null);
  const [recordToDelete, setRecordToDelete] = useState<LeasedBuildingOut | null>(null);
  ...
  const recordsSafe = Array.isArray(leasedBuildingsOut) ? leasedBuildingsOut : [];
  ...
  const openAddForm = () => {
    ...
    setFormOpen(true);
    setTimeout(() => document.getElementById('leased-building-out-form')?.scrollIntoView({ behavior: 'smooth', block: 'start' }), 50);
  };

  const openEditForm = (record: any) => {
    if (!hasPermission('leased_buildings_out', 'canEdit')) {
  ...
  const openDetails = (record: LeasedBuildingOut) => {
    setSelectedRecord(record);
    setFormOpen(false);
    setDetailsOpen(true);
    setTimeout(() => document.getElementById('leased-building-out-details')?.scrollIntoView({ behavior: 'smooth', block: 'start' }), 50);
  };

  const requestDelete = (record: LeasedBuildingOut) => {
  ...
  const confirmDelete = async () => {
    ...
    try {
      await deleteLeasedBuildingOut(recordToDelete.id);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (formMode === 'add') {
        await addLeasedBuildingOut(payload as any);
        toast.success('تمت إضافة السجل بنجاح');
      } else if (selectedRecord) {
        await updateLeasedBuildingOut(selectedRecord.id, payload as any);
  ...
  return (
    <div className="container mx-auto p-4 md:p-6 space-y-6">
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl md:text-3xl font-bold">المباني المؤجرة من الجامعة</h1>
          <p className="text-muted-foreground mt-1">إدارة عقود تأجير مباني الجامعة للمستأجرين مع المواقع والمرفقات.</p>
        </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>ابحث باسم المستأجر أو رقم العقد أو المبنى أو الموقع.</CardDescription>
        </CardHeader>
                ...
                <CardContent className="flex h-full flex-col space-y-4 p-4 sm:p-5">
                  <div className="flex items-start justify-between gap-3">
                    <div className="min-w-0">
                      ...
                      <h3 className="mt-1 break-words text-lg font-black text-slate-800">{record.locationName || record.buildingNumber || record.tenant?.name || 'مبنى مؤجر'}</h3>
                      <p className="mt-1 text-xs text-muted-foreground">المستأجر: {record.tenant?.name || '-'}</p>
                    </div>
                    <Badge variant="outline" className="shrink-0 border-sky-300 bg-sky-50 text-sky-700">المستأجر</Badge>
                  </div>
                  ...
                  <div className="platform-record-actions">
                    ...
                    {hasPermission('leased_buildings_out', 'canEdit') && (
      ...
      {detailsOpen && selectedRecord && (
        <div id="leased-building-out-details" className="rounded-xl border bg-card p-4 md:p-6 shadow-sm">
          <div className="mb-5 flex flex-col md:flex-row md:items-center justify-between gap-3">
            <div>
              <h2 className="text-xl md:text-2xl font-bold">تفاصيل المبنى المؤجر</h2>
          ...
          <div className="space-y-5">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-3">
              <div>
                <h3 className="text-xl font-bold">{(selectedRecord as any).locationName || 'مبنى مؤجر'}</h3>
