...
const getFieldVisitCategoryBadgeClass = (category: string) => {
...
type QuranInventoryConsistency = {
  valid: boolean;
  total: number | null;
  withdrawal: number;
  message: string | null;
...
export const MosqueFieldVisitsPanel: React.FC<Props> = ({ sites, currentUsername, canAdd, canEdit, canDelete, canPrint }) => {
  ...
  const getSelectedQuranRackStock = (itemType: QuranEquipmentItemType) => quranRackDashboard?.sites.find((row) => row.site.id === visitForm.siteId && row.itemType === itemType) || null;
  ...
  const updateQuranInventoryDetails = (index: number, patch: Partial<MosqueFieldVisitQuranInventoryDetails>) => {
  ...
  for (const rackCensusItem of rackCensusItems) {
    ...
    try {
      ...
      const payload = {
        ...
        teamMembers,
        items: preparedVisitItems.map((item) => ({
          ...item,
      ...
      if (!quranSyncResult?.needsCorrection) await syncQuranSupplyRequest(savedVisit, refreshedQuranStock);
      setVisitDialog(false);
      await load();
    ...
    {view === 'visits' ? <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
      {filteredVisits.map((visit) => <Card key={visit.id} className="overflow-hidden border-slate-200 shadow-sm">
        <CardHeader className="border-b bg-gradient-to-l from-slate-50 to-white pb-4">
          <div className="flex items-start justify-between gap-3"><div><CardTitle className="text-base">{visit.site.name}</CardTitle><CardDescription className="mt-1">{visit.visitNumber}</CardDescription></div><div className="flex flex-col items-end gap-1.5"><StatusBadge status={visit.workflowStatus} />{visit.canEdit !== true && <Badge variant="outline" className="border-slate-300 bg-slate-100 text-slate-700"><Eye className="ml-1 h-3 w-3" />عرض فقط</Badge>}{quranVisitNeedsCorrection(visit) && <Badge className="bg-amber-600 text-white"><AlertTriangle className="ml-1 h-3 w-3" />جرد المصاحف يحتاج تصحيح</Badge>}</div></div>
        </CardHeader>
        <CardContent className="space-y-3 pt-4 text-sm">
          ...
          <div className="flex flex-wrap justify-end gap-2 border-t pt-3">
            ...
            {canDelete && visit.canDelete === true && <Button size="sm" variant="destructive" className="!border-red-700 !bg-red-600 !text-white shadow-sm hover:!bg-red-700 hover:!text-white" onClick={() => setDeletingVisit(visit)}><Trash2 className="ml-1 h-4 w-4 text-white" />حذف</Button>}
          </div>
        </CardContent>
      </Card>)}
        ...
        {viewingVisit && <>
          ...
          <div className="space-y-3">
            ...
            {viewingVisit.items.map((item, index) => <Card key={item.id || `${item.category}-${item.title}-${index}`} className={item.status === 'needs_action' ? 'border-amber-300 bg-amber-50/30' : ''}>
              <CardContent className="space-y-3 pt-4">
                <div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
                  <div><Badge variant="outline" className={getFieldVisitCategoryBadgeClass(item.category)}><span aria-hidden className="h-1.5 w-1.5 rounded-full bg-current opacity-70" /><span aria-hidden className="pointer-events-none absolute inset-x-2 top-0 h-px bg-white/90" /><span className="relative">{item.category}</span></Badge><p className="font-bold text-slate-800">{item.title}</p></div>
                ...
                {isQuranFieldVisitItem(item) && quranInventorySummary(item) && <ReadOnlyNote label="الجرد الميداني للمصاحف" value={quranInventorySummary(item)} />}{isQuranRackFieldVisitItem(item) && quranRackSummary(item) && <ReadOnlyNote label={`جرد ${quranEquipmentLabel(quranEquipmentItemType(item))}`} value={quranRackSummary(item)} />}
      ...
      <DialogContent className="sm:max-w-[540px]" dir="rtl">
        {tourAction && <>
          <DialogHeader className="text-right">
            ...
            <DialogDescription>{tourAction.action === 'delete' ? `سيتم حذف الجولة ${tourAction.tour.tourNumber} والزيارات المجدولة التابعة لها. الحذف متاح لمنشئ الجولة التي أنشأها بنفسه، أو لمسؤول المنصة الذي يملك الصلاحية الكاملة.` : `سيتم إلغاء الجولة ${tourAction.tour.tourNumber} مع الاحتفاظ بسجل الجولة والزيارات المرتبطة بها لأغراض التوثيق والمراجعة.`}</DialogDescription>
          </DialogHeader>
