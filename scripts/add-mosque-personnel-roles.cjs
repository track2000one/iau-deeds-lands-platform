...
// Add selected personnel role state for operational assignment editor.
...
// After assignments load, initialize drafts.
...
// Replace setUserAssignment helper with exact role/site/personnelRole support.
...
// If role options are written as map-less single line, normalize collaborator label globally in personnel context.
...
// Replace raw personnel role badge/text with labels where common patterns exist.
...
// Build a richer roles tab if the current one contains the known simple assignment control.
...
// Enhance personnel card section description and role labels, if present.
s = s.replace('إضافة منسوب مسجد', 'إضافة منسوب مسجد');
