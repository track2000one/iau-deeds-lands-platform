...
upload_close = """    return body as { driveUrl: string; driveFileId?: string; fileName?: string; mimeType?: string };
  },
};"""
upload_with_delete = """    return body as { driveUrl: string; driveFileId?: string; fileName?: string; mimeType?: string };
  },
...
if upload_close not in api:
    raise RuntimeError('mosque API upload anchor not found')
api = api.replace(upload_close, upload_with_delete, 1)

react_old = "import React, { useEffect, useMemo, useState } from 'react';"
react_new = "import React, { useEffect, useMemo, useRef, useState } from 'react';"
if react_old not in page:
    raise RuntimeError('React import anchor not found')
page = page.replace(react_old, react_new, 1)

xlsx_import = "import * as XLSX from 'xlsx';"
if xlsx_import not in page:
    raise RuntimeError('XLSX import anchor not found')
page = page.replace(xlsx_import, xlsx_import + "\nimport JSZip from 'jszip';", 1)

export_anchor = "\n\nexport const MosquesUnitPage: React.FC = () => {"
if export_anchor not in page:
    raise RuntimeError('component export anchor not found')
helpers = r'''
...
page = page.replace(export_anchor, helpers + export_anchor, 1)

state_anchor = "  const [siteMediaLibrary, setSiteMediaLibrary] = useState<MosqueSiteMediaLibrary>(emptySiteMedia());"
if state_anchor not in page:
    raise RuntimeError('site media state anchor not found')
...
page = page.replace(state_anchor, state_insert, 1)
...
report_anchor = "  const exportReportExcel = async () => {"
if report_anchor not in page:
    raise RuntimeError('report function anchor not found')
import_functions = r'''  const openMediaImportDialog = () => {
    setMediaImportRows([]);
...
'''
page = page.replace(report_anchor, import_functions + report_anchor, 1)

add_button = r'''{canAdd && ['head', 'supervisor'].includes(role) && <Button className={`${button3d} bg-sky-700 hover:bg-sky-800`} onClick={() => openSiteDialog()}><Plus className="ml-2 h-4 w-4" />إضافة مسجد / مصلى</Button>}'''
if add_button not in page:
    raise RuntimeError('add site button anchor not found')
import_button = r'''{canEdit && ['head', 'supervisor'].includes(role) && <Button variant="outline" className={button3d} onClick={openMediaImportDialog}><FileText className="ml-2 h-4 w-4" />استيراد مكتبة ZIP</Button>}
            ''' + add_button
page = page.replace(add_button, import_button, 1)
...
site_dialog_anchor = '      <Dialog open={siteDialog} onOpenChange={setSiteDialog}>'
if site_dialog_anchor not in page:
    raise RuntimeError('site dialog anchor not found')
import_dialog = r'''      <Dialog open={mediaImportDialog} onOpenChange={(open) => { if (!mediaImportSaving) { setMediaImportDialog(open); if (!open) { setMediaImportRows([]); mediaImportZipRef.current = null; setMediaImportProgress({ done: 0, total: 0, label: '' }); } } }}>
...
'''
page = page.replace(site_dialog_anchor, import_dialog + site_dialog_anchor, 1)

preview_anchor = r'''            {previewSite.notes && <div className="rounded-2xl border bg-slate-50 p-4"><p className="text-xs text-muted-foreground">ملاحظات</p><p className="mt-2 whitespace-pre-wrap text-sm leading-7 text-slate-700">{previewSite.notes}</p></div>}
...
if preview_anchor not in page:
    raise RuntimeError('preview media anchor not found')
preview_with_media = r'''            {previewSite.notes && <div className="rounded-2xl border bg-slate-50 p-4"><p className="text-xs text-muted-foreground">ملاحظات</p><p className="mt-2 whitespace-pre-wrap text-sm leading-7 text-slate-700">{previewSite.notes}</p></div>}
            ...
            {previewSite.latitude != null && previewSite.longitude != null && <div className="flex justify-end"><Button variant="outline" className={button3d} onClick={() => window.open(`https://www.google.com/maps?q=${previewSite.latitude},${previewSite.longitude}`, '_blank')}><MapPin className="ml-2 h-4 w-4" />فتح الموقع على الخريطة</Button></div>}'''
page = page.replace(preview_anchor, preview_with_media, 1)

preview_class_old = 'className="sm:max-w-[720px]">\n          <DialogHeader className="text-right">\n            <DialogTitle className="flex items-center gap-2 text-xl font-black"><Eye className="h-5 w-5 text-sky-700" />معاينة — {previewSite?.name}</DialogTitle>'
preview_class_new = 'className="max-h-[90vh] overflow-y-auto sm:max-w-[900px]">\n          <DialogHeader className="text-right">\n            <DialogTitle className="flex items-center gap-2 text-xl font-black"><Eye className="h-5 w-5 text-sky-700" />معاينة — {previewSite?.name}</DialogTitle>'
if preview_class_old in page:
    page = page.replace(preview_class_old, preview_class_new, 1)

api_path.write_text(api, encoding='utf-8')
page_path.write_text(page, encoding='utf-8')
print('Applied reviewed mosque ZIP media import and preview support.')
