...
util = r'''import ExcelJS from 'exceljs';
...
Path('src/app/utils/excelReport.ts').write_text(util, encoding='utf-8')
...
def patch(path_str, replacements):
    ...
    for old, new in replacements:
        if old not in text:
            raise SystemExit(f'Expected text not found in {path_str}: {old[:120]}')
        text = text.replace(old, new)
    path.write_text(text, encoding='utf-8')

patch('src/app/pages/MosquesUnitPage.tsx', [
    ("import { appendExcelReportSheet, excelReportDateStamp } from '../utils/excelReport';", "import { appendExcelReportSheet, excelReportDateStamp, writeProfessionalExcel } from '../utils/excelReport';"),
    ('const exportQuranWarehouseExcel = (warehouse: MosqueQuranWarehouse) => {', 'const exportQuranWarehouseExcel = async (warehouse: MosqueQuranWarehouse) => {'),
    ('const exportQuranInventoryExcel = () => {', 'const exportQuranInventoryExcel = async () => {'),
    ("const exportSitesExcel = (rows: MosqueSite[], filePrefix = 'mosques-sites-report') => {", "const exportSitesExcel = async (rows: MosqueSite[], filePrefix = 'mosques-sites-report') => {"),
    ("XLSX.writeFile(workbook, `quran-warehouse-${warehouse.code || warehouse.id}-${excelReportDateStamp()}.xlsx`);", "await writeProfessionalExcel(workbook, `quran-warehouse-${warehouse.code || warehouse.id}-${excelReportDateStamp()}.xlsx`, { title: `بطاقة مكتبة المصاحف — ${warehouse.name}`, subtitle: 'الرصيد الحالي وحدود الأمان وحركة المكتبة', orientation: 'landscape', imageLoader: async (fileId, url) => fileId ? mosqueApi.mediaBlob(fileId) : (url ? fetch(url).then((response) => response.ok ? response.blob() : null) : null) });"),
    ("XLSX.writeFile(workbook, `quran-inventory-report-${excelReportDateStamp()}.xlsx`);", "await writeProfessionalExcel(workbook, `quran-inventory-report-${excelReportDateStamp()}.xlsx`, { title: 'تقرير إدارة وحصر المصاحف', subtitle: `عدد المواقع: ${quranPrintRows.length}`, orientation: 'landscape', metrics: [{ label: 'المواقع', value: quranPrintStats.sites, tone: 'blue' }, { label: 'إجمالي المصاحف', value: quranPrintStats.total, tone: 'green' }, { label: 'الكبيرة', value: quranPrintStats.large }, { label: 'المتوسطة', value: quranPrintStats.medium }, { label: 'الصغيرة', value: quranPrintStats.small }, { label: 'المسحوبة', value: quranPrintStats.damaged, tone: 'red' }, { label: 'الاحتياج', value: quranPrintStats.needed, tone: 'amber' }], imageLoader: async (fileId, url) => fileId ? mosqueApi.mediaBlob(fileId) : (url ? fetch(url).then((response) => response.ok ? response.blob() : null) : null) });"),
    ("XLSX.writeFile(workbook, `${filePrefix}-${excelReportDateStamp()}.xlsx`);", "await writeProfessionalExcel(workbook, `${filePrefix}-${excelReportDateStamp()}.xlsx`, { title: rows.length === 1 ? `بطاقة ${rows[0].name}` : 'جدول المساجد والمصليات الجامعية', subtitle: `عدد المواقع: ${rows.length}`, orientation: selectedColumns.length > 6 ? 'landscape' : 'portrait', imageLoader: async (fileId, url) => fileId ? mosqueApi.mediaBlob(fileId) : (url ? fetch(url).then((response) => response.ok ? response.blob() : null) : null) });"),
    ("XLSX.writeFile(workbook, `mosques-unit-report-${excelReportDateStamp()}.xlsx`);", "await writeProfessionalExcel(workbook, `mosques-unit-report-${excelReportDateStamp()}.xlsx`, { title: 'التقرير الشامل لوحدة العناية بالمساجد والمصليات الجامعية', subtitle: 'تجميع بيانات المواقع والطلبات والبلاغات والإجازات والتوظيف والمصاحف', orientation: 'landscape', imageLoader: async (fileId, url) => fileId ? mosqueApi.mediaBlob(fileId) : (url ? fetch(url).then((response) => response.ok ? response.blob() : null) : null) });"),
])

patch('src/app/components/MosqueFieldVisitsPanel.tsx', [
    ("import { appendExcelReportSheet, excelReportDateStamp } from '../utils/excelReport';", "import { appendExcelReportSheet, excelReportDateStamp, writeProfessionalExcel } from '../utils/excelReport';"),
    ('const exportVisitExcel = (visit: MosqueFieldVisit) => {', 'const exportVisitExcel = async (visit: MosqueFieldVisit) => {'),
    ('const exportProgramExcel = () => {', 'const exportProgramExcel = async () => {'),
    ('const exportTourTreatmentExcel = (tour: MosqueFieldTour) => {', 'const exportTourTreatmentExcel = async (tour: MosqueFieldTour) => {'),
    ("XLSX.writeFile(workbook, `field-visit-${visit.visitNumber}-${excelReportDateStamp()}.xlsx`);", "await writeProfessionalExcel(workbook, `field-visit-${visit.visitNumber}-${excelReportDateStamp()}.xlsx`, { title: visitPrintTitle.trim() || `تقرير زيارة ميدانية — ${visit.site.name}`, subtitle: `${visit.visitNumber} — ${new Date(visit.visitDate).toLocaleDateString('ar-SA-u-ca-gregory')}`, orientation: 'landscape', imageLoader: async (fileId, url) => fileId ? mosqueApi.mediaBlob(fileId) : (url ? fetch(url).then((response) => response.ok ? response.blob() : null) : null) });"),
    ("XLSX.writeFile(workbook, `field-program-report-${excelReportDateStamp()}.xlsx`);", "await writeProfessionalExcel(workbook, `field-program-report-${excelReportDateStamp()}.xlsx`, { title: programReportTitle.trim() || 'تقرير البرنامج الميداني للمساجد والمصليات', subtitle: `عدد الزيارات: ${filteredVisits.length}`, orientation: 'landscape', metrics: [{ label: 'إجمالي المواقع', value: summary.totalSites, tone: 'blue' }, { label: 'تمت زيارتها', value: summary.visitedSites, tone: 'green' }, { label: 'المتبقية', value: summary.remainingSites, tone: 'amber' }, { label: 'نسبة التغطية', value: `${summary.coveragePercent}%`, tone: 'green' }, { label: 'الملاحظات المفتوحة', value: summary.openItems, tone: 'amber' }, { label: 'عاجلة', value: summary.urgentItems, tone: 'red' }], imageLoader: async (fileId, url) => fileId ? mosqueApi.mediaBlob(fileId) : (url ? fetch(url).then((response) => response.ok ? response.blob() : null) : null) });"),
    ("XLSX.writeFile(workbook, `field-tour-${tour.tourNumber}-${excelReportDateStamp()}.xlsx`);", "await writeProfessionalExcel(workbook, `field-tour-${tour.tourNumber}-${excelReportDateStamp()}.xlsx`, { title: `تقرير المعالجة المصور — ${tour.title}`, subtitle: `${tour.tourNumber} — سجل قبل / بعد المعالجة`, orientation: 'landscape', imageLoader: async (fileId, url) => fileId ? mosqueApi.mediaBlob(fileId) : (url ? fetch(url).then((response) => response.ok ? response.blob() : null) : null) });"),
])

print('Professional Excel report patch applied.')
